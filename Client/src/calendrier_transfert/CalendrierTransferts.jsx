  ...
  const mergeTransfersAndInventories = (transfersResponse, inventoriesResponse,manualTransfersResponse) => {
    ...
    if (transfersResponse?.data) {
      transfersResponse.data.forEach((transfer) => {
        const dateKey = formatDateToKey(transfer.Date); // Utiliser Date avec majuscule
        ...
        mergedData[dateKey].transfers.push({
          ...
          date: formatDateString(transfer.Date), // Format DD/MM/YYYY pour l'affichage
          Date: transfer.Date, // Conserver la date ISO pour les mises à jour
        });
      });
    }
...
const updateInventory = async (id, inventoryData) => {
  ...
  // Supprimer un inventaire
    ...
    const handleUpdateTransfer = async (date, updatedItem) => {
      try {
        // Fonction pour convertir le format de date DD/MM/YYYY ou YYYY-MM-DD vers YYYY-MM-DD
        const formatDateForAPI = (dateStr) => {
          if (!dateStr) return new Date().toISOString().split('T')[0]; // Date par défaut si vide
          // Si la date est déjà au format YYYY-MM-DD
          if (/^\d{4}-\d{2}-\d{2}/.test(dateStr)) return dateStr;
          // Sinon, convertir DD/MM/YYYY vers YYYY-MM-DD
        ...
        const formattedData = {
          ...updatedItem,
          Date: formatDateForAPI(updatedItem.date), // Utiliser "Date" avec majuscule pour correspondre au schéma
        ...
        if (updatedItem.showBoxIcon) {
          ...
          await updateInventory(updatedItem._id, {
            date: formattedData.Date, // Utiliser le format ISO
              ...
              transfers: prevData[dateStr].transfers.map((t) =>
                t._id === updatedItem._id
                  ? {
                      ...t,
                      ...updatedItem,
                      Date: formattedData.Date, // Mettre à jour avec la nouvelle date
                      type: getTypeFromStatus(updatedItem.status),
